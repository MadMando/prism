...
\date{May 2026 — revised for v0.2.13}
...
bonus — operationalizing the memory consolidation insight that knowledge
...
\begin{itemize}[leftmargin=*]
  ...
  \item An open-source Python library (\texttt{prism-rag} on PyPI, v0.2.13)
...
\textsc{derives\_from}  & positive    & 0.85 & $p_i$ is logically derived from $p_j$ \\
\textsc{specializes}    & positive    & 0.80 & $p_i$ is a specific instance of $p_j$ \\
\textsc{implements}     & positive    & 0.75 & $p_i$ concretely realizes the concept in $p_j$ \\
\textsc{exemplifies}    & positive    & 0.75 & $p_i$ is a concrete example illustrating $p_j$ \\
\textsc{generalizes}    & positive    & 0.70 & $p_i$ is a broader abstraction of $p_j$ \\
\textsc{qualifies}      & qualifying  & 0.65 & $p_i$ adds conditions or exceptions to $p_j$ \\
\textsc{contrasts\_with}& dialectical & 0.55 & $p_i$ and $p_j$ take different positions \\
\textsc{refutes}        & dialectical & 0.50 & $p_i$ directly contradicts $p_j$ \\
...
For each passage $p_i$, we retrieve its $k$-nearest neighbors by
...
\paragraph{Step 3: Graph serialization.}
The completed graph is serialized to gzip-compressed JSON and stored
...
Given a query $q$, we initialize a per-node activation score.
...
to its graph neighbors, weighted by edge type and attenuated by a
global decay factor $\delta$:
\[
  a^{(t)}(p_j) = \sum_{(p_i, p_j, r) \in \mathcal{E}}
    a^{(t-1)}(p_i) \cdot w(r) \cdot c_{ij} \cdot \delta^t
\]
where $w(r)$ is the propagation weight for edge type $r$ and $c_{ij}$ is
the LLM-assigned confidence for that edge (stored as a single combined
weight $w(r) \cdot c_{ij}$ in the serialized graph).  Activation
\emph{accumulates} across all incoming neighbors, so nodes corroborated
by multiple paths receive proportionally higher scores.  Propagation also
traverses edges in the \emph{reverse} direction (from $p_j$ back to
$p_i$), scaled by a reverse-edge penalty of $0.6$ to prevent
backward-flow from dominating forward evidence.
...
\[
  f(p_j) = a(p_j) \cdot \bigl(1 + \lambda \cdot \text{conv}(p_j)\bigr)
\]
where $\lambda$ is the convergence weight (default $\lambda = 0.4$).
This multiplicative formulation preserves the full magnitude of
activation while applying a convergence \emph{bonus}: a node reached by
every seed is scaled by up to $1 + \lambda = 1.4$, whereas a node
activated by a single seed receives no bonus.  Nodes with high
convergence — corroborated by multiple independent query perspectives —
are thus promoted without suppressing strongly-activated unique matches.
...
Table~\ref{tab:comparison} summarizes how PRISM differs from major
...
distribution of a random walk, marginalizing over all starting points.
...
frameworks, practitioner guides). Using $k=8$ nearest neighbors per
...
PRISM is optimized for multi-source corpora. The \texttt{cross\_source\_only}
